...
static PROBLEM: LazyLock<[Coordinate; PROBLEM_LEN]> = LazyLock::new(|| {
    let mut problem_to_solve: [Coordinate; PROBLEM_LEN] = [Coordinate { x: 0.0, y: 0.0 }; PROBLEM_LEN];
    let mut rng = rand::rng();

    for index in 0..PROBLEM_LEN {
    	problem_to_solve[index] = Coordinate {x: rng.random::<f64>(), y: rng.random::<f64>()};
    }
...
fn test_traveling_salesman() {
    run_simulation(new_genome, score_function, output, vec![mutate, mutate2, mutate3], finish);
	println!("Unoptimized sample distance was {}", distance(&new_genome()));
	greedy_solve();
...
fn distance(genome: &[usize; PROBLEM_LEN]) -> f64 {
    ...
    for index in 1..genome.len() {
        dist += ((PROBLEM[genome[index]].x - PROBLEM[genome[index - 1]].x).powi(2)
        	+ (PROBLEM[genome[index]].y - PROBLEM[genome[index - 1]].y).powi(2)).sqrt();
    }

    dist += ((PROBLEM[genome[0]].x - PROBLEM[genome[genome.len()-1]].x).powi(2)
        	+ (PROBLEM[genome[0]].y - PROBLEM[genome[genome.len()-1]].y).powi(2)).sqrt();
...
fn output(genome: &[usize; PROBLEM_LEN]) {
    println!("{:?}", genome);
    println!("Distance of TSP path calculated with the genetic algorithm {}", distance(genome));
}
...
fn greedy_solve() {
	...
	while !nodes_left.is_empty() {
		...
		for index in 0..nodes_left.len() {
			let tempdist = ((nodes_left[index].x - current_node.x).powi(2)
				+ (nodes_left[index].y - current_node.y).powi(2)).sqrt();
	...
	distance_answer += ((nodes_store[0].x - current_node.x).powi(2)
				+ (nodes_store[0].y - current_node.y).powi(2)).sqrt();

	println!("Distance of TSP problem path calculated with greedy algorithm {:?}", distance_answer);
}
